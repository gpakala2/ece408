...
__global__ void conv_forward_kernel(float *output, const float *input, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    for (int c = 0; c < Channel; c++) { // sum over all input channels
        if((h < Height) && (w < Width))
            tile[threadIdx.y * SIZE + threadIdx.x] = in_4d(b, c, h, w);
        else
        ...
        if(threadIdx.y < TILE_WIDTH && threadIdx.x < TILE_WIDTH){
            for (int i = 0; i < K; i++) // loop over KxK filter
                for (int j = 0; j < K; j++)
                    acc += tile[(threadIdx.y + i) * SIZE + threadIdx.x + j] * mask_4d(m, c, i, j);
        }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    size_t tile_size = SIZE * SIZE * sizeof(float);

    conv_forward_kernel<<<gridDim, blockDim, tile_size>>>(device_output, device_input, Batch, Map_out, Channel, Height, Width, K);
}
