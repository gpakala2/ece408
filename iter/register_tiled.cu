...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    int Width_grid = ceil(Width_out / (1.0*TILE_SZ_A));    // Number of horizontal tiles for output maps
    int Height_grid = ceil(Height_out / (1.0*TILE_SZ_B));    // Numer of vertical tiles for output maps

    int Out_grid = Height_grid * Width_grid;   // Y-Dimension of the grid
    //int SIZE = TILE_WIDTH + K - 1;

    dim3 gridDim(Width_grid, Width_grid, Batch);
    dim3 blockDim(TILE_SZ_A, 1, 1);
    size_t tile_size = Channel * TILE_SZ_RATIO * TILE_SZ_B * sizeof(float);
